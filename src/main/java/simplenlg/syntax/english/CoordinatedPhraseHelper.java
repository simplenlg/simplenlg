	...
	static NLGElement realise(SyntaxProcessor parent, CoordinatedPhraseElement phrase) {
		...
		if(phrase != null) {
			...
			if(children != null && children.size() > 0) {
				...
				if(phrase.getFeatureAsBoolean(Feature.RAISE_SPECIFIER).booleanValue()) {
				...
				for(int index = 1; index < children.size(); index++) {
					...
					setChildFeatures(phrase, child);
					if(phrase.getFeatureAsBoolean(Feature.AGGREGATE_AUXILIARY).booleanValue()) {
	...
	private static void raiseSpecifier(List<NLGElement> children) {
		...
		if(child != null) {
			...
			if(specifier != null) {
				...
				// baseform
				test = (specifier instanceof WordElement) ? ((WordElement) specifier).getBaseForm() :
			...
			if(test != null) {
				...
				while(index < children.size() && allMatch) {
					...
					if(child == null) {
						...
						specifier = child.getFeatureAsElement(InternalFeature.SPECIFIER);
						String childForm =
								(specifier instanceof WordElement) ? ((WordElement) specifier).getBaseForm() :
										specifier.getFeatureAsString(LexicalFeature.BASE_FORM);
