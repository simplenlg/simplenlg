...
public class ClauseAggregationTest extends SimpleNLG4Test {
	...
	protected void setUp() {
		...
		this.s1.addPostModifier(this.phraseFactory.createPrepositionPhrase("behind",
		...
		this.s3.addPostModifier(this.phraseFactory.createPrepositionPhrase("behind",
		                                                                   this.phraseFactory.createNounPhrase("the",
		                                                                                                       "curtain")));
		...
		this.s6.addPostModifier(this.phraseFactory.createPrepositionPhrase("behind",
		                                                                   this.phraseFactory.createNounPhrase("the",
		                                                                                                       "curtain")));
	}
	...
	public void testCoordinationSameSubjectFail() {
		List<NLGElement> elements = Arrays.asList((NLGElement) this.s1, (NLGElement) this.s2);
	...
	public void testCoordinationSameVP() {
		List<NLGElement> elements = Arrays.asList((NLGElement) this.s3, (NLGElement) this.s4);
		List<NLGElement> result = this.coord.apply(elements);
		Assert.assertTrue(result.size() == 1); // should only be one sentence
		NLGElement aggregated = result.get(0);
		Assert.assertEquals("the woman and the man kick the dog behind the curtain", //$NON-NLS-1$
		                    this.realiser.realise(aggregated).getRealisation());
	}
	...
	public void testCoordinationWithModifiers() {
		...
		Assert.assertTrue(result.size() == 1); // should only be one sentence
		NLGElement aggregated = result.get(0);
		Assert.assertEquals("however the woman and the man kick the dog behind the curtain", //$NON-NLS-1$
		                    this.realiser.realise(aggregated).getRealisation());
	}
	...
	public void testCoordinationSameVP2() {
		List<NLGElement> elements = Arrays.asList((NLGElement) this.s3, (NLGElement) this.s4, (NLGElement) this.s5);
		List<NLGElement> result = this.coord.apply(elements);
		Assert.assertTrue(result.size() == 1); // should only be one sentence
		NLGElement aggregated = result.get(0);
		Assert.assertEquals("the woman and the man and the girl kick the dog behind the curtain", //$NON-NLS-1$
		                    this.realiser.realise(aggregated).getRealisation());
	}
	...
	public void testForwardConjReduction() {
		...
		Assert.assertEquals("the woman kicks the dog on the rock and kicks the dog behind the curtain", //$NON-NLS-1$
		                    this.realiser.realise(aggregated).getRealisation());
	}
	...
	public void testBackwardConjunctionReduction() {
		...
		Assert.assertEquals("the woman kicks and the woman kisses the dog behind the curtain",
		                    this.realiser.realise(aggregated).getRealisation());
	}
